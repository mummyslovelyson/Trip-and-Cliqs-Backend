...
// In-memory cache for system settings to eliminate database latency on regular requests
const cache = new Map();
let lastFetch = 0;
    ...
    if (!inFlightFetch) {
      inFlightFetch = (async () => {
        try {
          ...
          // If system_settings fails or table doesn't exist, back off for 15s to prevent slamming the DB
          lastFetch = Date.now() - (TTL - 15000);
        } finally {
          inFlightFetch = null;
        }
      })();
    }
    // If cache already has data, return immediately while background refresh finishes, otherwise await
    if (cache.size === 0 && inFlightFetch) {
      await inFlightFetch;
    }
  }
